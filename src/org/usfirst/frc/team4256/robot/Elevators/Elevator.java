...
public class Elevator implements Subsystem {
	...
	public Elevator(L_One one, L_Two two) {
		this.one = one;
		this.two = two;
	}
...
//}
	...
	**/
	private double validateInches(final double inches) {
		final double minimumHeight = climbing ? L_One.hookBaseline : 0.1;//ideally 0, but .1 helps avoid encoder noise around 0
		final double maximumHeight = climbing ? L_One.hookBaseline + L_One.maximumHeight : L_One.maximumHeight + L_Two.maximumHeight;
	...
	public void increment(final double inches) {
		if (climbing) setInches(currentSetpoint + inches + L_One.hookBaseline - two.getInches());
	...
	**/
	private void setClampHeight(double desiredInches) {
		...
		if (desiredInches > currentElOnePosition) {
			...
			if (desiredInches <= currentElOnePosition + L_Two.maximumHeight) {
				two.setInches(desiredInches - one.getInches());
			}else {
				two.setInches(L_Two.maximumHeight);
	...
	**/
	private void setHookHeight(double desiredInches) {
		desiredInches = validateInches(desiredInches);
		final double elevatorOneDesiredInches = desiredInches - L_One.hookBaseline;
	...
	public void enableClimbMode(final Clamp clamp) {
		two.setInches(L_Two.climbingHeight);
		clamp.close();
		clamp.rotateTo(90.0);
		one.setTorque(true);
		climbing = true;
